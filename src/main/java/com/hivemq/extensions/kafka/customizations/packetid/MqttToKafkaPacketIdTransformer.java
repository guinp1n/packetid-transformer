 ...
 * @author Georg Held
 */
public class MqttToKafkaPacketIdTransformer implements MqttToKafkaTransformer {

    private static final @NotNull Logger log = LoggerFactory.getLogger(MqttToKafkaPacketIdTransformer.class);
    ...
    public void init(final @NotNull MqttToKafkaInitInput input) {
        ...
        log.info(
                "PackeId-Transformer for Kafka cluster '{}' with boot strap servers '{}' initialized.",
                kafkaCluster.getId(),
    ...
    public void transformMqttToKafka(
        ...
        // convert MQTT user properties to Kafka headers
        publishPacket.getUserProperties()
                .asList()
                .forEach(userProperty -> {
                    log.info("User property: " + userProperty.getName() + ", value: " + userProperty.getValue());
                    recordBuilder.header(userProperty.getValue(), userProperty.getValue());
                });
